@extends('layouts.app')
@section('title','Online Person Information')
@section('css')
        ...
        <div class="content-wrapper bg-light">
          <div class="page-header">
            <h2 class="page-title p-3">
              ...
              </span><span class="fs-4">Online Person Informations</span>
            </h2>
          </div>
          <div class="row">
            <div class="col-12 grid-margin">
                <div class="container-fluid">
                  ...
                  <div class="table-responsive">
                    <div>
                      <h4>Appointed date and time search</h4>
                      <select id="dropdown1" name="date" class="date">
                        <option value="" disabled selected="true">Select Date</option>
                        @foreach ($date as $item)
                          <option value="{{$item->appointed_date}}">{{$item->appointed_date}}</option>                          
                        @endforeach
                      </select>
  
                      <select id="dropdown2" name="time" class="time">
                        <option value="" disabled selected="true">Select Time</option>
                        @foreach ($time as $times)
                          <option value="{{$times->from_time}}">{{$times->from_time}}</option>
                        @endforeach
                      </select>
                    </div>
                    <table class="table table-bordered table-striped" style="white-space: nowrap" id="onlinePersonInformationTable">
                      <thead>
                        <tr>
                          <th> No </th>
                          <th> Person Code </th>
                      ...
                      <tbody>
                        @php
                          $i=1;
                        @endphp
                        @foreach ($p_information as $person)
                        <tr>
                          <td> {{$i++}} </td>
...
@push('script')
    ...
    <script>
      $(document).ready(function() {
        var table =  $('#onlinePersonInformationTable').DataTable();
